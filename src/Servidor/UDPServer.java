			...
			while (true) {
				if (!fez) {
					...
					if (info.equals("b")) {
						...
						DatagramPacket msgBroad = new DatagramPacket(msfinal, msfinal.length, recebimento.getAddress(),
								recebimento.getPort());
						fez = true;
					...
					if (info.equals("c")) {
						...
						byte[] msfinal = msgrecebe.getBytes();
						DatagramPacket msgchat = new DatagramPacket(msfinal, msfinal.length, recebimento.getAddress(),
								recebimento.getPort());
						System.out.println("-------------------------------------------");
...
class ThreadBroadcast implements Runnable {
	...
	String[] conectados = new String[1024];
	Scanner entrada = new Scanner(System.in);
	byte[] buffer2 = new byte[1024];
	int cont = 0;
	...
	public void run() {
		...
		String mac = info[0];
		String nome = info[1];
		if (conectados[cont] == null) {
			conectados[cont] = nome + "/n";
			cont = cont++;

		} else {
			conectados[cont] = conectados[cont] + nome + "/n";
			 cont = cont++;
		...
		// }
		System.out.println("conectados: " + conectados[cont]);

		// System.out.println("Deseja conectar-se com " + conectados[cont] );
		String desejo = "sim";
